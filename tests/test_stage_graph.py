...
BOOK = {
    ...
    "tools": ["prepare_booking", "confirm_action", "search_availability"],
    "clears": ["slot"],
}
...
def two_stage_state() -> CallGraph:
    return CallGraph.start(parse_graph(TWO_STAGE), call_id="CA-1")
...
def run_turn(prompt, state, client, repository, tool_outputs=None):
    ...
    original_tools = module.ClinicTools

    class StubApi:
        def close(self):
            pass

    class StubClinicTools:
    ...
    module.ClinicTools = StubClinicTools
    try:

        async def drive():
            return [
                r.immediate_answer
                async for r in run_agent_turn(
        ...
        module.ClinicTools = original_tools
...
def test_shipped_graphs_use_the_prepare_then_confirm_flow():
    for path in (Path("graphs/default.json"), Path("graphs/clinic.json")):
        graph = load_graph(path)
        tool_names = {tool for node in graph.nodes for tool in node.tools}
        ...
        assert "confirm_action" in graph.node("confirmar").tools
        assert {
            "book_appointment",
...
def test_call_graph_owns_the_request_context_and_turn_clock():
    state = CallGraph.start(parse_graph(TWO_STAGE), call_id="CA-77")

    state.start_turn()
    state.start_turn()

    assert state.context.call_id == "CA-77"
    assert state.turn == state.context.turn_number == 2
...
def test_record_facts_cannot_invent_a_proposal_id():
    state = two_stage_state()

    assert state.refuse_reason("record_facts", {"facts": {"proposal_id": "prop_fake"}})
...
def test_a_forbidden_tool_is_refused_and_the_promise_is_never_spoken():
    ...
    client = FakeClient(
        says("I'll book that right now.", ("prepare_booking", {"slot_id": "slot_1"})),
    ...
    assert "prepare_booking" not in client.offered[0]
...
def test_one_refused_call_suppresses_the_whole_utterance():
    ...
    client = FakeClient(
        says(
            ...
            ("prepare_booking", {"slot_id": "slot_1"}),
        ),
...
def test_a_facts_only_step_ends_the_turn_so_the_caller_can_answer():
    ...
    client = FakeClient(
        says(
            "What day suits you?", ("record_facts", {"facts": {"specialty": "cardio"}})
        ),
    )
...
def test_a_tool_result_continues_the_turn_without_the_caller():
    ...
    client = FakeClient(
        ...
        says(
            "",
            ("record_facts", {"facts": {"patient_id": "P1"}}),
            ("go_to", {"stage": "book"}),
        ),
...
def test_graph_api_round_trips_and_refuses_a_broken_graph(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        api_module, "save_graph", lambda graph: graph_module.save_graph(graph, path)
    )
    ...
    assert len(client.get("/api/tools").json()["tools"]) == 13

    broken = {
        "entry": "identify",
        "nodes": [IDENTIFY],
        "edges": [{"from": "identify", "to": "gone"}],
    }
    ...
    renamed = {
        **TWO_STAGE,
        "nodes": [{**IDENTIFY, "prompt": "Ask for their ID."}, BOOK],
    }
...
def test_a_failed_submission_is_not_retried_in_the_same_turn():
    ...
    repository = FakeRepository()
    client = FakeClient(
        says("Confirming that.", ("confirm_action", {"proposal_id": "prop_1"})),
        says("Trying again.", ("confirm_action", {"proposal_id": "prop_2"})),
    ...
    spoken = run_turn(
        ...
        client,
        repository,
        {"confirm_action": RuntimeError("Response timed out after request acceptance")},
    )
...
def test_a_second_booking_in_the_same_batch_is_refused_after_the_first_fails():
    ...
    client = FakeClient(
        says(
            "Booking that.",
            ("confirm_action", {"proposal_id": "prop_1"}),
            ("confirm_action", {"proposal_id": "prop_2"}),
        ),
    ...
    run_turn("book me in", state, client, repository, {"confirm_action": timing_out})
...
def test_a_failed_lookup_may_be_tried_again_but_a_failed_submission_may_not():
    ...
    state.failed_tools.add("confirm_action")

    assert state.refuse_reason("search_patients", {"name": "Ana"}) == ""
    state.enter("book")
    assert state.refuse_reason("confirm_action", {"proposal_id": "prop_1"})
...
def test_a_failed_call_is_not_remembered_as_already_answered():
    ...
    spoken = run_turn(
        "Soy Ana", state, client, FakeRepository(), {"search_patients": flaky}
    )
