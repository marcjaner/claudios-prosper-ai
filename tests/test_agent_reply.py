...
def test_agent_reply_speaks_immediate_and_grounded_answers():
    async def run():
        calls = []
        ...
        def runner(prompt, client, *, clinic_api, call_id):
            ...
            yield ToolResult(name="search_availability", output={"slots": []})
            yield "There are no slots available in that date range."
    ...
    assert "Verified clinic tool results" not in prompt
    assert '"name": "search_availability"' in calls[1][0]
    assert '"slots": []' in calls[1][0]
    assert client is not None
    assert calls[1][1] is client
    assert captured_api is clinic_api
    assert call_id == "CA456"
    assert isinstance(emitted[0], TTSSpeakFrame)
    assert emitted[0].text == "Let me check that for you."
    assert isinstance(emitted[1], TTSSpeakFrame)
    assert emitted[1].text == "There are no slots available in that date range."
