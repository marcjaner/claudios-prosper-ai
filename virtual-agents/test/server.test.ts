...
import { createDashboardStore, type DashboardCall } from "../src/dashboard-store.js";
...
test("SQLite dashboard storage survives a full server restart", { timeout: 5000 }, async () => {
  const directory = mkdtempSync(join(tmpdir(), "virtual-agents-dashboard-"));
  const database = join(directory, "dashboard.sqlite");
  const callId = "persisted-dashboard-call";
  const ready = Promise.withResolvers<void>();
  const first = createVoiceServer(config(), async (call) => {
    call.onRecord?.({ type: "transcript", speaker: "user", itemId: "persisted-user", text: "Persistent question" });
    call.onRecord?.({ type: "transcript", speaker: "assistant", itemId: "persisted-agent", text: "Persistent answer" });
    ready.resolve();
    return { sendAudio() {}, sendText() {}, async close() {} };
  }, "console", undefined, createDashboardStore(database));
  const firstPort = await first.listen();
  try {
    const client = new WebSocket(`ws://127.0.0.1:${firstPort}/ws`);
    await once(client, "open");
    client.send(JSON.stringify(start(callId)));
    await ready.promise;
    const ended = once(client, "close");
    client.send(JSON.stringify({ event: "stop", streamSid: `stream-${callId}` }));
    await ended;
  } finally {
    await first.close();
  }
  ...
  const interruptedId = "interrupted-dashboard-call";
  const seeder = createDashboardStore(database);
  seeder.saveCall({
    call_id: interruptedId, started_at: 1, ended_at: null, state: "speaking", from_number: null,
    patient_id: null, patient_name: null, insurer: null, outcome: null, reason: null, cost_eur: null,
    ttfa_seconds: null, error: null, score_overall: null, score_json: null, score_error: null,
    guardrail_breached: false, guardrail_reason: null, guardrail_violations: null,
  } satisfies DashboardCall);
  seeder.close();

  const second = createVoiceServer(config(), async () => {
    throw new Error("Persisted dashboard reads must not start a voice session");
  }, "console", undefined, createDashboardStore(database));
  const secondPort = await second.listen();
  try {
    const detail: unknown = await fetch(`http://127.0.0.1:${secondPort}/api/calls/${callId}`)
      .then((response) => response.json());
    const restored = z.object({
      call: z.object({ call_id: z.literal(callId), state: z.literal("ended") }),
      events: z.array(z.object({ kind: z.string(), payload: z.record(z.string(), z.unknown()) })),
    }).parse(detail);
    assert.deepEqual(restored.events.filter((event) => event.kind === "stt_final" || event.kind === "tts")
      .map((event) => event.payload.text), ["Persistent question", "Persistent answer"]);
    const interrupted: unknown = await fetch(`http://127.0.0.1:${secondPort}/api/calls/${interruptedId}`)
      .then((response) => response.json());
    const interruptedCall = z.object({ call: z.object({
      state: z.string(), error: z.string(), ended_at: z.number(),
    }) }).parse(interrupted).call;
    assert.equal(interruptedCall.state, "error");
    assert.equal(interruptedCall.error, "server_restarted");
    assert.ok(interruptedCall.ended_at > 1);
    const health: unknown = await fetch(`http://127.0.0.1:${secondPort}/healthz`).then((response) => response.json());
    assert.equal(z.object({ dashboardStorage: z.string() }).parse(health).dashboardStorage, "sqlite");
  } finally {
    await second.close();
...
async function exerciseConcurrentCalls(count: number, recordAudio = false): Promise<void> {
  const directory = recordAudio ? mkdtempSync(join(tmpdir(), "virtual-agents-parallel-audio-")) : undefined;
  ...
  try {
    ...
    assert.deepEqual(health, {
      ...
      voiceProvider: "azure", voiceConnector: "realtime",
      voiceDeployment: settings.AZURE_OPENAI_DEPLOYMENT, voiceOutputGainDb: 0,
    });
    if (directory) {
...
test("Live health and output use the independent profile instead of Realtime gain", { timeout: 5000 }, async () => {
  ...
  try {
    const health = z.object({
      voiceProvider: z.string(), voiceConnector: z.string(), voiceDeployment: z.string(),
      voiceBackendDeployment: z.string(), voiceOutputGainDb: z.number(),
    }).parse(await fetch(`http://127.0.0.1:${port}/healthz`).then((response) => response.json()));
    assert.deepEqual(health, {
      voiceProvider: "azure", voiceConnector: "live", voiceDeployment: "gpt-live-1",
      voiceBackendDeployment: "gpt-5.4-mini", voiceOutputGainDb: 0,
    });
    const client = new WebSocket(`ws://127.0.0.1:${port}/ws`);
    await once(client, "open");
    const response = once(client, "message");
...
test("headerless upgrades are accepted and invalid media does not start an Azure session", { timeout: 5000 }, async () => {
  let opened = false;
  const settings = config();
  const server = createVoiceServer(settings, async () => {
    opened = true;
...
test("records transcripts and accepted actions privately through the actual server lifecycle", { timeout: 5000 }, async () => {
  const directory = mkdtempSync(join(tmpdir(), "virtual-agents-recorded-server-"));
...
test("an unconfirmed proposal gets one bounded reprompt that caller speech cancels", { timeout: 5000 }, async (t) => {
  const ready = Promise.withResolvers<Parameters<VoiceFactory>[0]>();
  const spoken: string[] = [];
  const server = createVoiceServer(config(), async (call) => {
    ready.resolve(call);
    return {
      sendAudio() {},
      sendText() {},
      requestSpeech(instructions) {
  ...
  try {
    const client = new WebSocket(`ws://127.0.0.1:${port}/ws`);
    await once(client, "open");
    client.send(JSON.stringify(start("confirmation-reprompt")));
    const call = await ready.promise;
    await delay(0);
    t.mock.timers.enable({ apis: ["setTimeout"] });
    call.onRecord?.({
      type: "action", stage: "proposed", proposalId: "booking-proposal",
      action: { action: "BOOK" },
    });
    call.onRecord?.({ type: "transcript", speaker: "assistant", itemId: "booking-readback", text: "May I book it?" });
    t.mock.timers.tick(12_000);
    assert.deepEqual(spoken, []);

    call.onRecord?.({
      type: "action", stage: "proposed", proposalId: "proposal-cancelled",
      action: { action: "REGISTER" },
    });
    call.onRecord?.({ type: "transcript", speaker: "assistant", itemId: "readback-1", text: "May I register you?" });
    call.onRecord?.({ type: "transcript", speaker: "user", itemId: "caller-replied", text: "One correction." });
    t.mock.timers.tick(12_000);
    assert.deepEqual(spoken, []);

    call.onRecord?.({
      type: "action", stage: "proposed", proposalId: "proposal-awaiting",
      action: { action: "REGISTER" },
    });
    call.onRecord?.({ type: "transcript", speaker: "assistant", itemId: "readback-2", text: "May I register you?" });
    t.mock.timers.tick(11_999);
    assert.deepEqual(spoken, []);
    t.mock.timers.tick(1);
    assert.equal(spoken.length, 1);
    assert.match(spoken[0]!, /Please say yes to register these details/);
    t.mock.timers.tick(12_000);
    assert.equal(spoken.length, 1);
    t.mock.timers.reset();

    const ended = once(client, "close");
    client.send(JSON.stringify({ event: "stop", streamSid: "stream-confirmation-reprompt" }));
    await ended;
  } finally {
    t.mock.timers.reset();
    await server.close();
...
test("records caller frames and only emitted agent frames in a private stereo WAV", { timeout: 5000 }, async () => {
  const directory = mkdtempSync(join(tmpdir(), "virtual-agents-audio-server-"));
