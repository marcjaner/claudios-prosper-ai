...
test("an explicit speech request creates one assistant response without inventing a caller turn", async (t) => {
  const h = harness(t);
  const session = await h.connect();
  assert.equal(session.requestSpeech?.("Say one short confirmation reminder."), true);
  const requests = h.socket.events("response.create");
  assert.equal(requests.length, 1);
  assert.deepEqual(requests[0]?.response, { instructions: "Say one short confirmation reminder." });
  assert.equal(h.records.filter((event) => event.type === "transcript" && event.speaker === "user").length, 0);
  assert.equal(session.requestSpeech?.("Do not overlap the active response."), false);
});
