...
test("function schemas are serializable and no model-provided call_id is accepted", async () => {
  ...
  assert.ok(JSON.stringify(receptionistTools).length < 16_000);
  const h = harness();
  await assert.rejects(h.execute("report_outcome", {
...
test("repairs harmless realtime-mini tool argument mistakes and records the repair", async () => {
  const h = harness();
  await identify(h);
  const result = z.object({ request_id: z.string() }).parse(await h.execute("search_availability", {
    patient_id: patient.patient_id,
    specialty_id: slot.specialty_id,
    prepare_booking: false,
    advance_day: false,
    request: { patient_id: patient.patient_id, specialty_id: slot.specialty_id },
  }));
  assert.equal(result.request_id, "request-1");
  const record = z.object({
    type: z.literal("tool"), name: z.literal("search_availability"), status: z.literal("ok"),
    details: z.object({ argument_repairs: z.array(z.object({ code: z.string(), field: z.string() })) }).passthrough(),
  }).parse(h.records.at(-1));
  assert.deepEqual(record.details.argument_repairs, [
    { code: "omitted_false_optional_flag", field: "prepare_booking" },
    { code: "omitted_false_optional_flag", field: "advance_day" },
    { code: "removed_redundant_request", field: "request" },
  ]);
});

test("prefers an explicit replacement date over realtime-mini advance_day noise", async () => {
  const h = harness({ slots: [] });
  await identify(h);
  const first = z.object({ request_id: z.string() }).parse(await h.execute("search_availability", {
    patient_id: patient.patient_id, specialty_id: slot.specialty_id, date_phrase: "Monday afternoon",
  }));
  await h.execute("search_availability", {
    patient_id: patient.patient_id, specialty_id: slot.specialty_id, request_id: first.request_id,
    date_phrase: "Tuesday afternoon", advance_day: true,
  });
  const record = z.object({
    details: z.object({ argument_repairs: z.array(z.object({ code: z.string(), field: z.string() })) }).passthrough(),
  }).parse(h.records.at(-1));
  assert.deepEqual(record.details.argument_repairs, [
    { code: "preferred_explicit_date", field: "advance_day" },
  ]);
});

test("keeps ambiguous registration arguments strict while omitting a harmless false flag", async () => {
  const h = harness();
  const started = z.object({ registration_id: z.string() }).parse(await h.execute("collect_registration", {
    new_registration: false,
  }));
  const repaired = z.object({
    details: z.object({ argument_repairs: z.array(z.object({ code: z.string(), field: z.string() })) }).passthrough(),
  }).parse(h.records.at(-1));
  assert.deepEqual(repaired.details.argument_repairs, [
    { code: "omitted_false_optional_flag", field: "new_registration" },
  ]);
  await assert.rejects(h.execute("collect_registration", {
    registration_id: started.registration_id, new_registration: true,
  }), { code: "invalid_tool_arguments" });
});

test("three failed lookups for the same ordinary booking give mini an exact recorded outcome", async () => {
  const h = harness({ patients: [] });
  const lookup = { name: "Missing Patient Person", national_id: "12345678Z" };
  await h.execute("find_patient", lookup);
  await h.execute("find_patient", lookup);
  const exhausted = z.object({
    identity_exhausted: z.literal(true),
    next_tool: z.object({
      name: z.literal("report_outcome"),
      arguments: z.object({ action: z.literal("NO_ACTION"), reason: z.literal("patient_not_found") }),
    }),
    instruction: z.string(),
  }).parse(await h.execute("find_patient", lookup));
  assert.match(exhausted.instruction, /do not suggest or start registration/i);
  assert.equal(h.writes.length, 0);
});
