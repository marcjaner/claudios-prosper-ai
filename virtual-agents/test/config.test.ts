...
test("configuration errors name fields but never expose values", () => {
  assert.throws(() => config({ AZURE_OPENAI_ENDPOINT: "https://example.org" }), (error: unknown) =>
    error instanceof Error && error.message.includes("AZURE_OPENAI_ENDPOINT") &&
      !error.message.includes("example.org"));
  assert.throws(() => config({ MAX_CONCURRENT_CALLS: "21" }));
});

test("OpenAI Realtime requires only its own key and rejects the Azure-only Live connector", () => {
  const direct = config({
    VOICE_PROVIDER: "openai", OPENAI_API_KEY: "private-openai-value", AZURE_OPENAI_ENDPOINT: undefined,
  });
  assert.equal(direct.VOICE_PROVIDER, "openai");
  assert.equal(direct.OPENAI_REALTIME_MODEL, "gpt-realtime-mini");
  assert.equal(direct.OPENAI_REALTIME_VOICE, "coral");
  assert.equal(direct.OPENAI_REALTIME_TRANSCRIPTION_MODEL, "whisper-1");
  assert.equal(direct.AZURE_OPENAI_ENDPOINT, undefined);
  assert.throws(() => config({
    VOICE_PROVIDER: "openai", OPENAI_API_KEY: undefined, AZURE_OPENAI_ENDPOINT: undefined,
  }), /OPENAI_API_KEY/);
  assert.throws(() => config({
    VOICE_PROVIDER: "openai", OPENAI_API_KEY: "key", VOICE_CONNECTOR: "live",
  }), /VOICE_CONNECTOR/);
  assert.throws(() => config({ VOICE_PROVIDER: "unknown" }), /VOICE_PROVIDER/);
});

test(".env.lang contributes only Azure settings; local and shell override it", () => {
  const directory = mkdtempSync(join(tmpdir(), "virtual-agents-config-"));
  try {
...
test("configuring output gain preserves endpoint credentials and rejects invalid changes", () => {
  const directory = mkdtempSync(join(tmpdir(), "virtual-agents-output-gain-"));
...
test("the explicit Live switch preserves Realtime settings and credentials through a switch back", () => {
  const directory = mkdtempSync(join(tmpdir(), "virtual-agents-voice-switch-"));
  const path = join(directory, ".env.local");
  const original = [
