...
test("Realtime remains the default and Live is never instantiated as a fallback", () => {
  ...
  assert.deepEqual(voiceProfile(settings), {
    provider: "azure", connector: "realtime", deployment: "gpt-realtime-1.5", outputGainDb: 9,
  });
});
...
test("Live uses independent settings and leaves the Realtime deployment/gain unchanged", () => {
  ...
  const factory = createConfiguredVoiceFactory(settings, new ProsperClient(settings), {
    azureRealtime: () => { throw new Error("No implicit fallback"); },
    live: () => unused,
  });
  assert.equal(factory, unused);
  ...
  assert.deepEqual(voiceProfile(settings), {
    provider: "azure", connector: "live", deployment: "live-custom", backendDeployment: "backend-custom", outputGainDb: 0,
  });
});
...
test("a Live constructor failure is surfaced, not converted into a Realtime call", () => {
  ...
  assert.throws(() => createConfiguredVoiceFactory(settings, new ProsperClient(settings), {
    azureRealtime: () => { throw new Error("Wrong fallback"); },
...
test("a different Realtime deployment changes only the existing Realtime profile", () => {
  ...
  assert.deepEqual(voiceProfile(settings), {
    provider: "azure", connector: "realtime", deployment: "gpt-realtime-2", outputGainDb: 0,
  });
});

test("OpenAI Realtime is explicit and leaves Azure available for an immediate switch back", () => {
  const settings = config({
    VOICE_PROVIDER: "openai", OPENAI_API_KEY: "test-openai-key", OPENAI_REALTIME_MODEL: "gpt-realtime-mini",
  });
  let openAI = 0;
  const factory = createConfiguredVoiceFactory(settings, new ProsperClient(settings), {
    azureRealtime: () => { throw new Error("Azure must not start"); },
    openAIRealtime: (received) => {
      assert.equal(received.OPENAI_REALTIME_MODEL, "gpt-realtime-mini");
      openAI += 1;
      return unused;
    },
  });
  ...
  assert.deepEqual(voiceProfile(settings), {
    provider: "openai", connector: "realtime", deployment: "gpt-realtime-mini", outputGainDb: 0,
  });
});
