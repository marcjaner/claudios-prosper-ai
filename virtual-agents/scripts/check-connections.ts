...
async function main(): Promise<void> {
    ...
    if (values.voice) {
      await withSpan("voice.connection_check", {}, ROOT_CONTEXT, async () => {
        ...
        try {
          ...
          log("info", "voice.provider_verified", {
  ...
  log("info", "connection_check.completed", {
    ...
    voice: values.voice ? "connected" : "not_checked",
    voiceAuthentication: values.voice
      ? config.VOICE_PROVIDER === "openai" || config.AZURE_OPENAI_API_KEY ? "api_key" : "DefaultAzureCredential"
      : "not_checked",
