...
async function main(): Promise<void> {
  ...
  const [{ createVoiceServer }, { createDashboardStore }, { createConfiguredVoiceFactory, voiceProfile }, { ProsperClient }] = await Promise.all([
    import("./server.js"),
    import("./dashboard-store.js"),
    import("./voice-provider.js"),
    import("./prosper.js"),
  ]);
  const server = createVoiceServer(
    config,
    createConfiguredVoiceFactory(config, new ProsperClient(config)),
    telemetry.mode,
    undefined,
