...
const descriptions: Record<keyof typeof toolSchemas, string> = {
  get_clinic: "Read official clinic facts and IDs. Omit section for a compact directory; request one section for details. Use all only when the full catalogue is truly necessary.",
...
const MODEL_SCHEMA_NOISE = new Set([
...
function compactModelSchema(value: unknown): unknown {
  ...
  return Object.fromEntries(Object.entries(value)
    .filter(([key]) => !MODEL_SCHEMA_NOISE.has(key))
    .map(([key, item]) => [key, compactModelSchema(item)]));
}
...
export const receptionistTools = Object.entries(toolSchemas).map(([name, schema]) => ({
  ...
  parameters: compactModelSchema(z.toJSONSchema(schema, { unrepresentable: "throw", io: "input" })),
}));
...
interface ArgumentRepair {
  code: "omitted_false_optional_flag" | "preferred_explicit_date" | "removed_redundant_request";
  field: string;
}
...
export class Receptionist {
  ...
  private readonly failedPatientLookups = new Map<string, number>();
  ...
  execute(name: string, argumentsJson: string, turn: number): Promise<unknown> {
    ...
    return withSpan(`execute_tool ${name}`, {
      ...
      try {
        ...
        ({ value: args, repairs } = this.repairToolArguments(name, args));
        ...
        switch (name) {
          case "get_clinic": {
            ...
            const facts = input.section === "all" ? summary
              : input.section ? { [input.section]: summary[input.section] }
              : {
                clinic_name: summary.clinic_name,
                calendar: summary.calendar,
                providers: summary.providers.map(({ id, name, specialty_id }) => ({ id, name, specialty_id })),
                locations: summary.locations.map(({ id, name, address }) => ({ id, name, address })),
                specialties: summary.specialties.map(({ id, name }) => ({ id, name })),
                plans: summary.plans.map(({ id, name }) => ({ id, name })),
                appointment_types: summary.appointment_types.map(({ id, name }) => ({ id, name })),
                instruction: "This is the compact directory. Request one section for detailed facts; use section all only when the caller truly needs the full catalogue.",
              };
            result = {
              ...facts,
        ...
        const decision = this.toolDecision(name, result);
        const details = repairs.length ? {
          ...(decision.details && typeof decision.details === "object" && !Array.isArray(decision.details)
            ? decision.details : {}),
          argument_repairs: repairs,
        } : decision.details;
        this.call.record({ type: "tool", name, status: "ok", ...(details === undefined ? {} : { details }) });
        return result;
      } catch (error) {
        let failure = error;
        ...
        const details = {
          ...
          ...(repairs.length ? { argument_repairs: repairs } : {}),
        };
        this.call.record({
  ...
  private repairToolArguments(name: string, value: unknown): { value: unknown; repairs: ArgumentRepair[] } {
    if (!value || typeof value !== "object" || Array.isArray(value)) return { value, repairs: [] };
    let repaired = { ...value } as Record<string, unknown>;
    const repairs: ArgumentRepair[] = [];
    const omitFalse = (fields: string[]) => {
      for (const field of fields) {
        if (repaired[field] !== false) continue;
        delete repaired[field];
        repairs.push({ code: "omitted_false_optional_flag", field });
      }
    };

    if (name === "collect_registration") omitFalse(["new_registration"]);
    if (name === "report_outcome") omitFalse(["no_other_policy"]);
    if (name === "search_availability") {
      omitFalse(["prepare_booking", "wait_for_provider_return", "new_request", "no_other_policy", "advance_day"]);
      if (repaired.advance_day === true &&
          ["date_phrase", "date_from", "date_to", "weekday"].some((field) => Object.hasOwn(repaired, field))) {
        delete repaired.advance_day;
        repairs.push({ code: "preferred_explicit_date", field: "advance_day" });
      }
      if (Object.hasOwn(repaired, "request")) {
        const candidate = { ...repaired };
        delete candidate.request;
        if (availabilityInput.safeParse(candidate).success) {
          repaired = candidate;
          repairs.push({ code: "removed_redundant_request", field: "request" });
        }
      }
    }
  ...
  private toolDecision(name: string, result: unknown): { details?: unknown } {
    ...
    if (name === "find_patient" && "matches" in result && Array.isArray(result.matches)) {
      const patient = result.matches.find((match: unknown) =>
        match && typeof match === "object" && "verified" in match && match.verified === true);
      if (patient && "patient_id" in patient && "name" in patient && "insurer" in patient) {
  ...
  private async findPatient(query: PatientQuery, turn: number): Promise<unknown> {
    ...
    const lookupKey = nameProvided ? normalizeHumanText(query.name!) : undefined;
    const failedAttempts = lookupKey && matches.length === 0
      ? (this.failedPatientLookups.get(lookupKey) ?? 0) + 1 : 0;
    if (lookupKey) {
      if (matches.length === 0) this.failedPatientLookups.set(lookupKey, failedAttempts);
      else this.failedPatientLookups.delete(lookupKey);
    }
    const identityExhausted = failedAttempts >= 3 && !this.registrationIntent;
    ...
    return {
      ...
      ...(identityExhausted ? {
        ...
        next_tool: { name: "report_outcome", arguments: { action: "NO_ACTION", reason: "patient_not_found" } },
      } : {}),
      ...(matches.length === 0 && this.registrationIntent ? {
        registration_intent: true,
        registration_ids: [...this.registrations.keys()],
      } : {}),
      instruction: identityExhausted
        ? "This ordinary booking has exhausted identity clarification. Call report_outcome with NO_ACTION patient_not_found now. Do not suggest or start registration unless the caller independently asks to register as a new patient."
        : matches.length === 0 && this.registrationIntent
  ...
  private async search(
    ...
    const slots = scan.slots.slice(0, MAX_RETURNED_SLOTS).map((slot) => {
