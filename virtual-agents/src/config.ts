...
const configSchema = z.object({
  ...
  VOICE_PROVIDER: z.enum(["azure", "openai"]).default("azure"),
  VOICE_CONNECTOR: z.enum(["realtime", "live"]).default("realtime"),
  AZURE_OPENAI_ENDPOINT: optionalHttpsOrigin.refine((value) =>
    value === undefined || new URL(value).hostname.endsWith(".openai.azure.com")),
  ...
  OPENAI_REALTIME_VOICE: z.string().min(1).default("coral"),
  OPENAI_REALTIME_TRANSCRIPTION_MODEL: z.string().min(1).default("whisper-1"),
  ...
  OTEL_SERVICE_NAME: z.string().min(1).default("virtual-agents"),
  TYPESAFE_API_KEY: optionalValue,
  ...
  if (value.VOICE_PROVIDER === "azure" && !value.AZURE_OPENAI_ENDPOINT) {
    context.addIssue({ code: "custom", path: ["AZURE_OPENAI_ENDPOINT"], message: "Required for Azure voice." });
  }
  if (value.VOICE_PROVIDER === "openai" && !value.OPENAI_API_KEY) {
    context.addIssue({ code: "custom", path: ["OPENAI_API_KEY"], message: "Required for OpenAI voice." });
  }
  if (value.VOICE_PROVIDER === "openai" && value.VOICE_CONNECTOR !== "realtime") {
    context.addIssue({ code: "custom", path: ["VOICE_CONNECTOR"], message: "OpenAI supports the Realtime connector." });
  }
...
export function readEnvironment(
  ...
  const shared = { ...read("../.env"), ...read(".env") };
  ...
  const overrides = { ...shared, ...local, ...definedShell };
  if (!overrides.PROSPER_API_BASE_URL && overrides.PLATFORM_API_BASE_URL) {
    overrides.PROSPER_API_BASE_URL = overrides.PLATFORM_API_BASE_URL;
  }
  if (!overrides.PROSPER_API_KEY && overrides.PLATFORM_API_KEY) {
    overrides.PROSPER_API_KEY = overrides.PLATFORM_API_KEY;
  }
