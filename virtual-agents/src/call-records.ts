...
export type CallRecordEvent =
  ...
  | { type: "rate_limit"; name: "requests" | "tokens"; limit: number; remaining: number; resetSeconds: number }
...
function eventFields(event: CallRecordEvent): Record<string, unknown> {
  ...
  switch (event.type) {
    ...
    case "rate_limit":
      if (event.name !== "requests" && event.name !== "tokens") {
        throw new RecordFailure("call_recording_invalid_event");
      }
      validCount(event.limit);
      validCount(event.remaining);
      if (!Number.isFinite(event.resetSeconds) || event.resetSeconds < 0) {
        throw new RecordFailure("call_recording_invalid_event");
      }
      return {
        type: event.type, name: event.name, limit: event.limit,
        remaining: event.remaining, resetSeconds: event.resetSeconds,
      };
    case "error":
