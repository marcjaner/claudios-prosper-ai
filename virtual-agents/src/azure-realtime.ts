...
export interface VoiceSession {
  ...
  requestSpeech?: (instructions: string) => boolean;
  close: () => Promise<void>;
}
...
export interface RealtimeProvider {
  ...
  session: (instructions: string) => Record<string, unknown>;
...
const rateLimitsSchema = z.object({
  rate_limits: z.array(z.object({
    ...
    reset_seconds: z.number().nonnegative(),
  })).max(8),
});
...
export function createAzureVoiceFactory(
  ...
  return createRealtimeVoiceFactory(prosper, {
    ...
    serverAddress: new URL(config.AZURE_OPENAI_ENDPOINT!).hostname,
    url: azureRealtimeUrl(config),
    headers(signal) {
      if (config.AZURE_OPENAI_API_KEY) return { "api-key": config.AZURE_OPENAI_API_KEY };
      return getToken(signal).then((token) => {
        if (!token) throw new AppError("azure_authentication_failed");
        return { Authorization: `Bearer ${token.token}` };
      }).catch(() => {
        throw new AppError(signal.aborted ? "call_cancelled" : "azure_authentication_failed");
      });
    },
...
export function createRealtimeVoiceFactory(
  prosper: ProsperClient,
  provider: RealtimeProvider,
  dependencies: Pick<AzureRealtimeDependencies, "createWebSocket"> = {},
): VoiceFactory {
  const createWebSocket = dependencies.createWebSocket ?? ((url, options) => new WebSocket(url, options));
  const providerError = (code: string) => `${provider.id}_${code}`;
  return (call) => withSpan(`${provider.id}.realtime.connect`, {
    "gen_ai.system": provider.system,
    "gen_ai.request.model": provider.model,
    "server.address": provider.serverAddress,
  }, call.parent, async () => {
    if (call.signal.aborted) throw new AppError("call_cancelled");
    const pendingHeaders = provider.headers(call.signal);
    const headers = pendingHeaders instanceof Promise ? await pendingHeaders : pendingHeaders;
    if (call.signal.aborted) throw new AppError("call_cancelled");
    return new Promise<VoiceSession>((resolve, reject) => {
      const socket = createWebSocket(provider.url, {
        headers,
      ...
      const deadline = setTimeout(() => fail(new AppError(providerError("session_timeout"))), 15_000);

      function send(event: Record<string, unknown>): void {
        if (closed || socket.readyState !== WebSocket.OPEN) throw new AppError(providerError("not_connected"));
        const payload = JSON.stringify(event);
        if (socket.bufferedAmount + Buffer.byteLength(payload) > 1024 * 1024) {
          throw new AppError(providerError("input_backpressure"));
        }
        socket.send(payload);
      }
      ...
      function audibleTurn(responseId: string): boolean {
        ...
        if (!turn) throw new AppError(providerError("unknown_response"));
      ...
      function fail(error: AppError): void {
        if (closed) return;
        reject(error);
        void session.close().catch(() => log("error", `${provider.id}.close_failed`, { callId: call.callId }));
        call.onFailure(error);
      }

      const session: VoiceSession = {
        sendAudio(payload) {
          if (!ready) throw new AppError(providerError("not_ready"));
        ...
        sendText(text) {
          if (closed) throw new AppError(providerError("not_connected"));
          if (!ready || pendingResponse !== undefined || hasActiveResponse() || pendingTools || userSpeaking) {
            throw new AppError(providerError("response_busy"));
          }
          dismissGreeting();
          generation += 1;
        ...
        requestSpeech(instructions) {
          return createResponse(instructions);
        },
        close() {
      ...
      async function handle(text: string): Promise<void> {
        ...
        event.type = provider.normalizeEventType?.(event.type) ?? event.type;
        switch (event.type) {
          ...
          case "error": {
            ...
            throw new AppError(code && /^[a-zA-Z0-9_]{1,64}$/.test(code)
              ? providerError(code) : providerError("service_error"));
          }
          ...
          case "rate_limits.updated": {
            const limits = rateLimitsSchema.parse(event).rate_limits;
            for (const limit of limits) {
              record({
                type: "rate_limit", name: limit.name, limit: limit.limit,
                remaining: limit.remaining, resetSeconds: limit.reset_seconds,
              });
            }
            break;
          }
          ...
          case "response.created": {
            ...
            if (turns.size >= 512) throw new AppError(providerError("response_limit"));
            const version = pendingResponse ?? generation;
            pendingResponse = undefined;
            dismissGreeting();
            const span = trace.getTracer("virtual-agents").startSpan("chat", {
              attributes: {
          ...
          case "response.function_call_arguments.done": {
            ...
            if (turn.done) {
              ...
              throw new AppError(providerError("tool_after_response"));
            }
          ...
          case "response.done": {
            ...
            if (!turn) throw new AppError(providerError("unknown_response"));
            if (turn.done) break;
            if (response.status === "in_progress") throw new AppError(providerError("invalid_event"));
            ...
            if (response.status === "failed" || response.status === "incomplete") {
              ...
              if (reason && /^[a-zA-Z0-9_]{1,32}$/.test(reason)) {
                record({ type: "error", code: providerError(`response_${response.status}_${reason}`) });
              }
              throw new AppError(providerError(`response_${response.status}`));
            }
      ...
      socket.on("open", () => {
        try {
          send({ type: "session.update", session: provider.session(instructions) });
        } catch (error) {
          fail(new AppError(errorCode(error)));
        }
      });
      socket.on("message", (data, isBinary) => {
        if (isBinary) return fail(new AppError(providerError("unexpected_binary")));
        void handle(data.toString()).catch((error: unknown) => {
          fail(error instanceof AppError ? error : new AppError(providerError("invalid_event")));
        });
      });
      socket.on("unexpected-response", (_request, response) => {
        response.resume();
        fail(new AppError(providerError(`http_${response.statusCode ?? 0}`)));
      });
      socket.on("error", () => fail(new AppError(providerError("websocket_error"))));
      socket.on("close", () => {
        if (!closed) fail(new AppError(providerError("disconnected")));
      });
    });
  });
}
