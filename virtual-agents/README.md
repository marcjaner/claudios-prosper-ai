# Virtual Agents - Realtime voice receptionist

A TypeScript backend for the Prosper HackSpain challenge. It adapts Prosper's
Twilio Media Streams protocol to Azure OpenAI Realtime in Foundry by default, or
directly to the OpenAI Realtime API through an explicit provider switch.
...
- Authenticated `/ws` endpoint, with one provider session per call.
...
- OpenTelemetry call, provider connection, response and tool spans, including
...
To use OpenAI directly instead, set these in `.env.local`:

```dotenv
VOICE_PROVIDER=openai
VOICE_CONNECTOR=realtime
...
`OPENAI_REALTIME_MODEL` is independent from the Azure deployment name. Switching
`VOICE_PROVIDER` back to `azure` immediately restores the Azure Realtime path;
there is intentionally no automatic failover during a live call.
...
Generate local configuration for the selected provider:

```sh
npm run configure
# If the Azure endpoint is not in either environment file:
npm run configure -- --endpoint https://your-resource.openai.azure.com
# Persist a provider switch without changing either provider's credentials:
npm run configure -- --voice-provider openai
npm run configure -- --voice-provider azure
```
...
Direct OpenAI uses `OPENAI_API_KEY` as a bearer token on the server-side
WebSocket. The key is never placed in the URL or sent to the browser.
...
`VOICE_PROVIDER` defaults to `azure`; `VOICE_CONNECTOR` defaults to `realtime`.
The direct OpenAI path supports the Realtime connector. The experimental `live`
path remains Azure-only in `src/azure-live.ts`. There is no automatic fallback
or mid-call model switching.
...
npm run start:openai         # Direct OpenAI Realtime
npm run start:azure          # Explicit return to Azure Realtime
...
The Prosper API key authenticates outbound clinic requests. Do not put it in a URL.
...
The local monitoring dashboard persists its call summaries, transcripts and
tool events in `.local/dashboard.sqlite` on every supported platform. The live
server still keeps the active view in memory, reloads it from SQLite after a
restart, and marks calls interrupted by a restart as `server_restarted`.
`/healthz` reports this as `dashboardStorage: "sqlite"`. The database does not
contain audio.
