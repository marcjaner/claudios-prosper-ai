...
export default function App() {
  ...
  const wallTheme = !openCall && view === "#/wall";

  return (
    <div className={`min-h-screen ${wallTheme ? "wall-theme bg-[#eef3f1] text-slate-900" : "bg-slate-950 text-slate-200"}`}>
      <header className={wallTheme
        ...
        <div className={`flex items-center ${wallTheme ? "justify-self-end gap-3" : "ml-auto gap-6"}`}>
          {!wallTheme && <Sparkline history={history} />}
          <div className="text-right">
            <p className={`${wallTheme ? "text-xl font-semibold text-emerald-600" : "font-mono text-3xl text-emerald-400"} tabular-nums`}>{live.length}</p>
            <p className={`text-xs ${wallTheme ? "text-slate-400" : "uppercase tracking-wide text-slate-500"}`}>
              {wallTheme ? "active" : "en curso"}
            </p>
          </div>
          <span
            className={`rounded-full px-3 py-1.5 text-xs font-medium ${
              connected
                ? wallTheme ? "bg-emerald-50 text-emerald-700" : "bg-emerald-500/10 text-emerald-400"
                : wallTheme ? "bg-rose-50 text-rose-700" : "bg-rose-500/10 text-rose-400"
            }`}
          >
            {connected
              ? wallTheme ? "connected" : "conectado"
              : wallTheme ? "reconnecting…" : "reconectando…"}
          </span>
        </div>
      </header>
