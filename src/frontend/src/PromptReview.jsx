...
export function ValidationDrawer({ report, isStale, isInspectorOpen, onClose, onNavigate }) {
  const counts = report.findings.reduce((result, finding) => {
    result[finding.severity] += 1;
    return result;
  }, { error: 0, warning: 0, suggestion: 0 });
  const scope = report.scope === "stage" ? report.stageId : "Agente completo";

  return (
    <aside
      aria-label="Resultados de validación"
      className={`validation-drawer-enter absolute inset-y-0 z-20 flex w-[30rem] flex-col border-l border-slate-200 bg-[#f7faf9] shadow-[-18px_0_45px_rgba(15,23,42,0.10)] transition-[right] duration-200 ${
        isInspectorOpen ? "right-96 max-w-[calc(100%-24rem)]" : "right-0 max-w-full"
      }`}
    >
      <header className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-600">{scope}</p>
          <div className="flex items-center gap-2">
            <h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-950">Validación</h2>
        ...
        <button type="button" onClick={onClose} aria-label="Cerrar validación" className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-xl text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700">×</button>
      </header>
      {report.findings.length === 0 ? (
        <div className="grid flex-1 place-items-center px-8 text-center">
          <div>
            ...
            <p className="mt-4 text-sm font-semibold text-emerald-800">La configuración es coherente.</p>
          </div>
        </div>
      ) : (
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          {report.findings.map((finding, index) => {
            ...
            return (
              <article key={`${finding.title}-${index}`} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="min-w-0 flex-1 text-sm font-semibold leading-5 text-slate-800">{finding.title}</h3>
                </div>
                <p className="mt-3 text-sm leading-6 text-slate-600">{finding.message}</p>
                {finding.suggestion && <p className="mt-2 text-sm leading-6 text-slate-400">{finding.suggestion}</p>}
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {finding.sources.map((source) => (
                      <button
                        ...
                        className="rounded-lg bg-slate-100 px-2.5 py-1.5 font-mono text-[11px] text-slate-500 transition-colors hover:bg-emerald-50 hover:text-emerald-700"
                      >
