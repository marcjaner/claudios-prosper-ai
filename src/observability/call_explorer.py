...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .call-filters { display:grid; grid-template-columns:1fr 1fr 1fr; gap:4px; margin-top:7px; }
    .call-filter { padding:6px 4px; border:1px solid var(--line); border-radius:4px; background:transparent; color:var(--muted); cursor:pointer; font-size:9px; text-transform:uppercase; }
    .call-filter.active { border-color:var(--green); background:#14201a; color:var(--green); }
    ...
    .connection-only .call-id { color:#78827e; }
    ...
    <aside>
      <header>
        ...
        <div class="call-filters">
          <button class="call-filter active" data-filter="runs">Agent runs</button>
          <button class="call-filter" data-filter="all">All attempts</button>
          <button class="call-filter" data-filter="connections">Connections</button>
        </div>
      </header>
      <div id="calls"></div>
    </aside>
  ...
  <script>
    ...
    function renderCalls() {
      ...
      const visible = calls.filter((call) => {
        const matchesId = call.call_id.toLowerCase().includes(needle);
        const run = isAgentRun(call);
        return matchesId && (callFilter === 'all' || (callFilter === 'runs' ? run : !run));
      });
      $('#calls').innerHTML = visible.map((call) => `
        <button class="call ${isAgentRun(call) ? '' : 'connection-only'} ${call.call_id === selected ? 'active' : ''}" data-call="${esc(call.call_id)}">
          <span class="call-id">${esc(call.call_id)}</span>
          <span class="call-meta"><span>${stamp(call.started_at)}</span><span>${isAgentRun(call) ? duration(call) : 'connection only'}</span></span>
    ...
    async function refreshCalls() {
      try {
        const response = await fetch(`${basePath}/api/calls`, {cache:'no-store'});
        calls = (await response.json()).calls;
        renderCalls();
        const runs = calls.filter(isAgentRun).length;
        $('#sync').textContent = `live · ${runs} runs / ${calls.length} attempts`;
    ...
    async function refreshDetail() {
      if (!selected) return;
      try {
        const response = await fetch(`${basePath}/api/calls/${encodeURIComponent(selected)}`, {cache:'no-store'});
